    ...
    <div className="container_global_country">
        ...
        if (index === index_country) {
          return (
            <div className="container_info_country" >
              ...
              <div className="args_country">Continente: {country.continent.name}</div>
              ...
              <div className="args_country">Población: {country.population}</div>
      ...
      {index != -1 &&
        <div className="ventana">
          ...
          <div className="datos">
            <div className="data">Continente: {data?.countries[index].continent.name}</div>
            <div className="data">Nombre: {data?.countries[index].name}</div>
            <div className="data">Codigo: {data?.countries[index].alpha2Code}</div>
            <div className="data">Población: {data?.countries[index].population}</div>
            ...
            {data?.countries[index].languages && data.countries[index].languages.map((language, index) => {
              return (<div className="data">Idioma {index}: {language.name}</div>)
            })}
            {data?.countries[index].currencies && data.countries[index].currencies.map((currency, index) => {
              return (<div className="data">Moneda {index}: {currency.name}</div>)
            })}
            {dataWeather &&
              <div>
                <br />
                <div className="data">Descripción del tiempo: {dataWeather.weather[0].description}</div>
                <div className="data">Tiempo: {(dataWeather.main.temp - 273.15)}ºC</div>
              </div>
            }
          </div>
        </div>
